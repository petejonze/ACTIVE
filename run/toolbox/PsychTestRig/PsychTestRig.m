    ...
    while (count <= nargin)
        ...
        elseif strncmpi(arg, '-createExp', n)
       	...
       	elseif strncmpi(arg, '-run', n)
            if count < nargin
                inputArgs = {varargin{count+1:end}}; % strip out the -run command
                %inputArgs = any2str(inputArgs); % convert any numeric inputs into strings
                % only convert numeric
                idx = cellfun(@isnumeric, inputArgs);
                if sum(idx)==1 
                    inputArgs{idx} = any2str(inputArgs{idx}); % crude!
                elseif sum(idx)>1
                    inputArgs(idx) = any2str(inputArgs{idx});
                end
