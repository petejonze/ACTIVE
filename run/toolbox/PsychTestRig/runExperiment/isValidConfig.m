...
%warning('PsychTestRig:isValidConfig','in progress - 2/2/10 - finish me')   
    ...
    if numParamsSpecified+2 < numParamsAccepted %+2 because first parameter is the metaInfo, and second parameter is runTimeInfo
       	fprintf(['      PTR_Error: The number of parameters specified in ' cfg.id ' (' int2str(numParamsSpecified) ') is fewer than the number required by ' cfg.script ' (' int2str(numParamsAccepted-2) ')\n'])
        isValid=false;
