...
% See also
    ...
    if (ismember(myField,Field)) %check that the field exists
        ...
        if (~all(checks))
            ...
            error(['/*****Setup failed. "' tableName '.' myField '" contains the following incorrectly specified properties(s): ' strjoin1(', ',failures{:}) '. You could try to manually remove this field, but this may cause the loss of existing data.*****/'])
        end   
    else %try to create
