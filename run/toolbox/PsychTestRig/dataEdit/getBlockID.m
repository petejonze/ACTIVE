...
% @Todo:            <blank>
    ...
    if ~strcmpi(blockID_name,blockID_meta)
        fprintf(['\n%s\n'...
                'Found BLOCK values did not match each other:'...
              	'\n                  From file name: %s'...
              	'\n   From file content (meta info): %s\n']...
              	,fn,blockID_name,blockID_meta)
        if getLogicalInput(sprintf('Replace file content value (%s) with file name value (%s)?',blockID_meta,blockID_name))
            expID = getMetaData(fn, 'Experiment ID');
            editMetaData(expID, fn,'Block Num', blockID_name);
        else
